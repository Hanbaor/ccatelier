...
export function initReader(){
 ...
 function apply(save=false){studio.style.setProperty('--reader-size',prefs.size+'px');studio.style.setProperty('--reader-leading',prefs.leading/10);studio.style.setProperty('--reader-width',prefs.width==='narrow'?'580px':'100%');$('[data-reader-size]').value=prefs.size;$('[data-reader-leading]').value=prefs.leading;$('[data-reader-width]').value=prefs.width;$('[data-size-output]').textContent=prefs.size;$('[data-leading-output]').textContent=(prefs.leading/10).toFixed(1);if(save&&!storage.set('cc-reader',JSON.stringify(prefs)))toast('阅读设置仅在当前页面有效');}
 ...
 // Keep the compact toolbar's existing font shortcut synchronized with the full desk.
 $('[data-reading-size]')?.addEventListener('click',()=>{prefs.size=prefs.size>=21?17:prefs.size+2;apply(true);});
